...
const objLat = (obj) => {
  let firstName =
    ...
    obj.firstName.slice(1, obj.firstName.length);
  let lastName =
    obj.lastName[0].toUpperCase() + obj.lastName.slice(1, obj.lastName.length);
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (item.yearsOfExperience > 1) {
      ...
      if (item.lastName && item.lastName !== "null") {
        if (item.firstName && item.firstName !== "null") fullName += " ";
...
const classesAvg = (data) => {
  for (let i = 0; i < data.grades.length; i++) {
    ...
    for (let j = 0; j < grade.classes.length; j++) {
      let item = grade.classes[j];
      let sum = 0;

      for (let k = 0; k < item.classScores.length; k++)
        sum += item.classScores[k];
      item.avg = Math.floor(sum / item.classScores.length);
    }
  }
  return data;
};
